...
class View{
    ...
    constructor( input_div_image, input_txt_cursor, input_div_hist, input_div_profilex, input_div_profiley, input_div_profilez ) {
...
class Controller{
    ...
    HoverEvent( event, ws ) {
        let x_px = Math.round( this.view.xmin + event.points[0].pointNumber[1]/this.view.rebin_ratio );
        let y_px = Math.round( this.view.ymin + event.points[0].pointNumber[0]/this.view.rebin_ratio );
    ...
    ChannelRequest( ws ) {
        ...
        let message = new proto.ImageViewer.Request();
        message.setEventType( 4 ); // CHANNEL
        message.setChannelRequestMessage( request_message );
        message.setSendStartTime( Date.now() );
    ...
    OnMessage( raw_message ) {
        ...
        } else if ( return_message.getEventType() == 4 ){ // CHANNEL
            this.ChannelResponse( return_message.getChannelResponseMessage() );
        }
    ...
    InitDisplayResponse( response_message ) {
        ...
        this.view.xmin         = response_message.getXmin();
        this.view.ymin         = response_message.getYmin();
        this.view.rebin_ratio  = response_message.getRebinRatio();

        this.view.hist_data    = response_message.getHistDataList();
    ...
    ChannelResponse( response_message ) {
...
// setup image display
let view = new View( divImage, txtCursor, divHist, divProfileX, divProfileY, divProfileZ );
