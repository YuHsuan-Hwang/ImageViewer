...
class Model:
    ...
    def InitDisplayResponse( self, message ):
        ...
        response_message.xmin        = self.xmin
        response_message.ymin        = self.ymin
    ...
    def ChannelResponse( self, message ):
        time1 = time.time()
        print("(", datetime.now(), ") start task: change channel" )

        self.channel = message.channel_request_message.channel
        ...
        return_message = pb.Response()
        return_message.event_type = pb.EventType.CHANNEL
        return_message.channel_response_message.CopyFrom( response_message )
        return_message.task_start_time = message.send_start_time
        return_message.send_start_time = round(time1*1000.0)
    ...
    try:
        ...
        async for message_bytes in websocket:
            ...
            if ( message.event_type==pb.EventType.INIT_DISPLAY ):
                ...
                return_message_bytes = model.ChannelResponse( message )
